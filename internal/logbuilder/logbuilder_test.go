...
func BenchmarkStaticFields(b *testing.B) {
	...
	l2 := NewLogBuilder()
	l2.AddField("time", time.Now().Format(time.RFC3339))
	l2.AddField("msg", "Hello, World!")
	l2.AddField("level", "INFO")
	l2.AddField("file", "logbuilder_test.go")
	l2.AddField("package", "logbuilder")
	l2.AddField("function", "BenchmarkStaticFields")
	l2.AddField("line", "30")

	b.Run("log builder add field function", func(b *testing.B) {
		for range b.N {
			l1.AddField("k", "v")
		}
	})
	...
	b.Run("comparation of json Marshal function", func(b *testing.B) {
		for range b.N {
			_, _ = json.Marshal(l2.fields)
		}
	})
}
